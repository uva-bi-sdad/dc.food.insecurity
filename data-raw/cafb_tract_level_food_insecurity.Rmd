...
cafb_tr_data <- st_drop_geometry(cafb3) %>% mutate(F15_FI_RATE = F15_FI_RATE * 100) %>%
  gather(measure, value, c(F15_FI_RATE, F15_FI_POP, F15_LB_NEED, F15_LB_UNME, F15_DISTRIB, F14_FI_RATE, F14_LB_UNME, F14_DISTRIB)) %>% # what is PPIN???
  rename(geoid = GEOID, region_name = NAME) %>%
  select(-c(F15_PPIN, F14_PPIN)) %>%
  mutate(measure_type = ifelse(measure %in% c("F15_FI_RATE", "F14_FI_RATE"), "percent",
                               ifelse(measure %in% c("F15_FI_POP"), "population", "weight")),
...
# merge(all.dat, dmv.tr, by.x = "geoid", by.y = "GEOID") %>% filter(year == 2019)
# 
# tm_shape(left_join(dmv.tr, all.dat, by = c("GEOID" = "geoid")) %>% filter(year == 2019), unit = "mi") +
...
#   tm_shape(cap_ct2) +
#   tm_polygons(alpha = 0) +
#   tm_layout(main.title = "2015 CAFB Food Insecurity Estimates",
